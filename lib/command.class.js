...
class Command {
  ...
  textCenter(text = '', replace = ' ') {
    let length = this._textLength / (this._state.fontSize + 1),
      textLength = this._computeTextLength(text),
      LRWidth = (length - textLength) / 2
    this._queue.push(
      ...iconv.encode(replace.repeat(LRWidth) + text + replace.repeat(LRWidth), this._encoding)
    )
    return this
  }
  ...
  textRow(texts = []) {
    ...
    texts.forEach((c, index) => {
      ...
      rowNum =
        Math.ceil((rowNum * 2 /* 乘以2，加入的间隔按中文长度来计算 */ + cWidth) / cellWidth) - 1
